...
them. This is where you come to *create*, *skillify*, or *skilltune* a suite skill — keep that
...
3. **`skills/suite/SKILL.md`** — the **authoring convention** (naming, scaffolder, symlink layout,
...
## 2. The work you do here — author and improve suite skills

Four operations, each owned by a skill. Pick by intent:

| Intent | Operation | Drive with |
|--------|-----------|------------|
| **New skill** from scratch or from a successful interactive session | Scaffold → author | `skills/suite/scripts/new-jstack-skill.sh <name> --desc "…"`, then `skill-creator` to write the `SKILL.md` body |
| **Codify** a working session into a new deterministic skill, or **harden** a fuzzy skill into scripts | Skillify (eval-gated) | `/skillify` |
| **Tune** an existing skill's accuracy/efficiency against an eval set | Skilltune (metric-driven loop) | `/skilltune <name>` |
| **Restructure / audit** an existing `SKILL.md` against the AgentSkills spec | Author | `skill-creator` |

**Naming convention (authoritative copy: `skills/suite/SKILL.md`):**
- Suite skills use short kebab-case names and live here.
- Venture-specific skills do not live here; they live in the venture repository (see §4). Never use a
  colon in a skill directory name.
...
that reaches "generate a video/clip" routes through `/vidgen`, never a backend directly.
...
|---|---|---|
| A **venture-agnostic** suite skill (voice, handoff, savetobrain, vision, skillify, skilltune, …) | **here** (`~/jstack/skills/`) | This is their canonical home. |
| A **StayFrame** (`jstack-sf-*`) skill, its craft canon, or its toolkit | **`~/jstack-sf`** (private venture repo, its own `AGENTS.md`) | Venture skills + IP live with the venture, not in the shareable suite. Start at `~/jstack-sf/skills/suite-sf/SKILL.md` (the router) and obey `~/jstack-sf/playbook/`. |
| **Knowledge** — a durable fact, decision, research synthesis, or "save this" | **`~/second-brain`** via `/savetobrain` | The brain stores knowledge; this repo stores skills. Never write the brain's `wiki/` from here. |
...
- **Eval-gate behavior changes.** `/skillify` and `/skilltune` are eval-gated so a skill's
