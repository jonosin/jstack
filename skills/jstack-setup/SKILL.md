---
name: jstack-setup
description: Install the jstack skill suite into an agent harness and write its config. Use when the user says /jstack-setup, set up jstack, install jstack, configure jstack, or has just cloned the jstack repo and wants it wired into Claude Code, Codex, or Hermes.
user_invocable: true
---

# jstack-setup — install and configure jstack

One-shot installer. Copies the `jstack-*` skills into the user's harness skill directory,
...
path, and GCP project (only if they want `jstack-vision`), then write them into `~/.jstack/config.env`.
...
| `SECOND_BRAIN_PATH` | jstack-brainwork, jstack-savetobrain* | `~/second-brain` |
| `GOOGLE_CLOUD_PROJECT` | jstack-vision | *(required for vision)* |
| `GOOGLE_CLOUD_LOCATION` | jstack-vision | `us-central1` |
| `GEMINI_VISION_MODEL` | jstack-vision | `gemini-2.5-flash` |
| `CLAUDE_PROJECTS_DIR` | jstack-claude-find, *-from-claude | `~/.claude/projects` |
| `JSTACK_HANDOFF_DIR` | jstack-handoff | `~/.jstack/handoffs` |
