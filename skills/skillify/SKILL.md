...
consistent / efficient" is **tuning, not hardening → `/skilltune`** (a metric eval loop, not a
...
Run this table once, whenever this skill's or skilltune's `description:` changes, to confirm the
...
| "make this skill more accurate" | Tune → `/skilltune` |
| "make this skill's output more consistent/reliable" | Tune → `/skilltune` |
...
- **New skill:** use `init_skill.py` to scaffold a staging dir under `/tmp/skillify-<name>/`. Place the codified script in `scripts/`, the fixture in `references/` or alongside the test.
...
skill-creator owns the authoring conventions. skillify owns the codification logic. Use both.
...
**Lint gate (both modes, mandatory).** Run `bash skills/suite/scripts/skill-lint.sh <target-skill-dir>`
...
- **New skill:** move the staging dir into `~/jstack/skills/<name>/`, then symlink it into `.agents`, `.claude`, `.codex`, and `.hermes/skills/suite/` per the jstack convention.
...
- **Metric-driven accuracy/efficiency tuning** of an existing skill — that is **`/skilltune`**.
...
| `/skilltune` | After hardening (or instead of it) to **measure + tune** accuracy/efficiency against a held-out eval set — the metric loop, not a refactor. |
| `/savetobrain` | The codified/hardened skill is durable knowledge worth ingesting into the second brain. |
