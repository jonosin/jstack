# focus — skilltune report

## Goal
Fix an over-compression regression introduced when `jstack-voice` + the old compression-layer
`focus` were merged. The merged skill redefined `tight` as "lead with what matters, then a line
...
## Follow-up
Run the full `/skilltune` loop under `/goal` in a Claude Code session with an independent Opus
