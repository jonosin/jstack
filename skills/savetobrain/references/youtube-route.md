...
```
python3 ~/jstack/skills/savetobrain/scripts/youtube-capture.py "<youtube_url>"
```
...
- Publishes only a prepared flat `raw/YYYY-MM-DD-<slug>-readable-transcript.md`
  artifact with the exact raw-v2 frontmatter
...
1. Get the scaffold (a few KB even for a 197KB clip):
   ```
   python3 ~/jstack/skills/savetobrain/scripts/youtube-capture.py scaffold "<clip_path>"
   ```
...
3. Write the picks to a temp JSON file as `[{"i": <segment index>, "heading": "<text>"}]`, then insert deterministically (headings inserted at byte offsets — body is never rewritten, so integrity is guaranteed):
   ```
   python3 ~/jstack/skills/savetobrain/scripts/youtube-capture.py insert "<clip_path>" "<picks_json_path>"
   ```
4. Report to user:
   ```
   Saved: raw/<file>.md — "<title>" (<channel>, <duration>, <N> sections)
   ```
...
Before script publication, call `capture prepare --route youtube` for the exact
flat target and pass its JSON with `--prepared`. After insertion, call `python3
tools/sb.py capture finalize --route youtube --artifact "raw/<file>.md" --json`
exactly once. No semantic extraction runs during capture.
