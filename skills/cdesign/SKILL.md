...
| `assets` | **Generate assets** (`/imgen` images, `/vidgen` clips) and feed them into a CD design | `references/uc-assets.md` |
...
- This skill does **not** write to the second brain. Durable knowledge is a separate `/savetobrain`.
...
- Never fabricate assets. Mark real vs placeholder; describe placeholders, generate real ones via
  `/imgen` / `/vidgen` (`uc-assets.md`), never fake them.
...
| `/imgen` | A use case needs generated images/textures/sprites to feed into the CD design (`uc-assets.md`). |
| `/vidgen` | The deliverable needs a generated video/clip (e.g. a reel) for an asset or the finishing pass. |
...
| `/savetobrain` | The session also produced durable decisions worth preserving in the brain (separate from this handoff). |
