...
Get leads from `linkedin-leads` (HarvestAPI, no login needed) or `linkedin-salesnav` (open Sales Nav tab). Or use the router: `linkedin`.
...
- Use `gtmarketing` (references/cold-outreach.md) for the framework and `myvoice` for voice (no em dashes).
...
| Next | When |
|------|------|
| `linkedin-leads` / `linkedin-salesnav` | Need more leads before building the next batch. |
