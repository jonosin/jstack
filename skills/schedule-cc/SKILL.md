...
Canonical scripts live at `~/jstack/skills/schedule-cc/scripts/` (symlinked into every
...
```bash
~/jstack/skills/schedule-cc/scripts/run_claude.sh \
...
~/jstack/skills/schedule-cc/scripts/schedule_claude.sh \
  --schedule "2h" \
...
- **Watch a run live** by tailing the detached log printed at schedule time:
  `tail -f ~/.hermes/logs/schedule-cc/jsc-<slug>.detached.log`.
...
- But two processes writing the same session transcript concurrently is unsafe (interleaved/corrupt
  ...
  `~/.hermes/logs/schedule-cc/<session-id>.log`.
