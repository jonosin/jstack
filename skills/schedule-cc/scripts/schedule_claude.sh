#!/usr/bin/env bash
# schedule-cc (scheduler) — register a Hermes cron job that runs a headless
...
# --- management modes --------------------------------------------------------
if [ -n "$LIST" ]; then
  echo "Scheduled claude jobs (name contains 'schedule-cc'):"
  hermes cron list 2>&1 | grep -iE 'schedule-cc|jsc-' || echo "(none)"
  exit 0
fi
if [ -n "$UNSCHED" ]; then
...
[ -n "$SESSION" ] && SESSION_ARG="--session \"$SESSION\" "
{
  ...
  echo "# Auto-generated by schedule-cc $(date '+%Y-%m-%d %H:%M:%S %Z'). Edit via the skill, not by hand."
  ...
  echo "#   tail -f $DETLOG"
  echo "mkdir -p \"$DETDIR\""
  echo "echo \"🟢 schedule-cc — '$JOBNAME' started \$(date '+%Y-%m-%d %H:%M %Z') · model $MODEL · dir $DIR. Detached; result posts here on completion.\""
