...
The generalized loop engine is **`/autoresearch`**; this skill is its skill-specific lens. To
*restructure* a skill or push fuzzy steps into deterministic code, use **`/skillify`** (harden) —
tuning never redesigns or widens scope.

**Invoke:** `/skilltune <existing-skill-name>`. The full S0–S6 loop mechanics live in
...
`references/eval/`, so the result survives even a closed tab. The sandbox is also a git repo: every S3
mutation commits before it's graded, so an interrupted run resumes by reading the last commit +
`<sandbox>/resume.json` (written at every stage transition) instead of a hand-written re-brief — see
"Git-native sandbox" (S0) and "Resuming an interrupted run" in `references/tune-mode.md`.
...
- **Show-eval** (default — a bare `/skilltune <name>` with no `/goal`/`auto`) — the ONE attended
  ...
  first recorded with `/to-spec`, then delegated with `/handoff goal`, not handled here.
  (Full detail: `references/tune-mode.md`.)
...
| `/autoresearch` | The generic metric loop engine this specializes — read for the underlying rules. |
| `/skillify` | You need to *restructure* the skill or push fuzzy steps into deterministic code (harden), not tune it. |
| `/goal` (or `/loop 30m /goal …`) | Run unattended: `/goal /skilltune <name>` is Auto mode — the loop runs itself end-to-end. |
| `/handoff` (goal) | Hand a fresh/unattended session the job of running the tune loop with approved evals. |
| `/savetobrain` | The tuned skill's eval results are durable knowledge worth ingesting into the second brain. |
