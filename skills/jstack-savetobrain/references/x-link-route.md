...
Preserve the external source as a raw clip in `raw/clips/` with minimal editorial judgment. Do not summarize into a conversation drop. Do not write wiki pages. Brainwork ingests later.
...
```yaml
---
title: "<post title or first meaningful line>"
source: "https://x.com/<handle>/status/<status_id>"
...
Add topic tags when obvious from the source, but do not overfit.
...
`check` must be clean before reporting success. `pending --json` supplies the final backlog count.

## Final response

Use the normal `jstack-savetobrain` two-line report:

```text
Saved raw source: raw/clips/<file>.md — <one-line description>. Brainwork deferred.
...
```text
Already saved: raw/clips/<file>.md. Brainwork still deferred.
Pending ingest backlog: <N> source(s). Run /jstack-brainwork to process.
```
