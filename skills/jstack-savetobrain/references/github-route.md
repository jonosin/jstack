# GitHub Repository Route — jstack-savetobrain

Use this route when the user explicitly saves a GitHub repository URL or an `owner/repo` slug. It
creates an immutable, compact `repository-guide-v2` raw source in `raw/clips/`; it never ingests or
writes `wiki/`.

## Preconditions

- The user has explicitly invoked `/jstack-savetobrain` or otherwise confirmed the save.
...
documentation, metadata, a tree, manifest contents, or code; fenced code blocks are rejected.

Pass each upstream document as a bounded source reference, with a one-line description:

```bash
python3 ~/jstack/skills/jstack-savetobrain/scripts/github-capture.py collect \
...
Pass at least one and at most 12 `--source-ref` values. Each reference must be a safe repository-relative regular
text path, optionally followed by ` :: ` and its description. The collector checks every reference
against the authenticated shallow clone, rejects secrets, binaries, generated/dependency paths, and
symlinks, then appends `## Where to go deeper` to the guide. Bare paths receive a generic upstream
reference description, but supply a specific description whenever possible.
...
`raw/clips/YYYY-MM-DD-github-owner-repo-1234567-guide-v2.md`
...
```
Saved raw source: raw/clips/<file>.md — GitHub repository <owner/repo> pinned at <7-char-sha>. Brainwork deferred.
