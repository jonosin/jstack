...
description: >
  ...
  browse the HyperFrames block catalog. NOT for generative video (that is /vidgen)
  and NOT for StayFrame static text overlays (that stays on /jstack-sf-caption).
---

# hyperframe — router to HeyGen HyperFrames
...
- **Generative video (i2v/t2v) is never HyperFrames** — that is `/vidgen`.
...
| Next | When |
|---|---|
| `/vidgen` | The composition needs generated footage (i2v/t2v clips) as source media — video generation always routes there, never direct to a backend. |
