...
```bash
OPENCLI_CDP_ENDPOINT=http://localhost:9222 OPENCLI_CDP_TARGET='sales/search' \
  ~/jstack/skills/linkedin-salesnav/scripts/salesnav-current-search.sh --limit 100 --out /tmp/salesnav-leads.json
```
...
| Next | When |
|------|------|
| `linkedin-send` | Extracted leads are ready to become a `prospects.csv` and get sent. |
