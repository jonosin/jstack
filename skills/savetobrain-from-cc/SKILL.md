...
python3 ~/.claude/skills/savetobrain-from-cc/scripts/extract_jstack_transcript.py <session-id>
```
...
The file is frontmattered per `skills/llm-wiki/references/raw-template.md` with `source: claude-code://session/<id>`, so `/brainwork` can ingest it as a Standard (or Personal) source.
...
```
Saved raw source: raw/personal/drops/2026-06-04-claude-session-0abed117.md. Brainwork deferred; run /brainwork when you want it processed.
```
...
Do not invoke `/brainwork` automatically. The growth-loop convention is to defer ingest. The user runs `/brainwork` when they want the drop processed.
...
| Next | When |
|------|------|
| `/brainwork` | After extracting the Claude session as a drop — process/ingest it (deferred by design; run when you want it compiled). |
| `/cc-find` | If you still need to locate the session ID to extract. |
