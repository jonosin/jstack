#!/usr/bin/env bash
# jstack-schedule-claude (scheduler) — register a Hermes cron job that runs a headless
...
# --- management modes --------------------------------------------------------
if [ -n "$LIST" ]; then
  echo "Scheduled claude jobs (name contains 'jstack-schedule-claude'):"
  hermes cron list 2>&1 | grep -iE 'jstack-schedule-claude|jsc-' || echo "(none)"
  exit 0
fi
if [ -n "$UNSCHED" ]; then
...
JOBNAME="${NAME:-jstack-schedule-claude:$SLUG}"
PROMPTFILE="$HSCRIPTS/jsc-$SLUG.prompt.txt"
WRAPPER="$HSCRIPTS/jsc-$SLUG.sh"
DETDIR="$HOME/.hermes/logs/jstack-schedule-claude"
...
[ -n "$SESSION" ] && SESSION_ARG="--session \"$SESSION\" "
{
  ...
  echo "# Auto-generated by jstack-schedule-claude $(date '+%Y-%m-%d %H:%M:%S %Z'). Edit via the skill, not by hand."
  ...
  echo "#   tail -f $DETLOG"
  echo "mkdir -p \"$DETDIR\""
  echo "echo \"🟢 jstack-schedule-claude — '$JOBNAME' started \$(date '+%Y-%m-%d %H:%M %Z') · model $MODEL · dir $DIR. Detached; result posts here on completion.\""
