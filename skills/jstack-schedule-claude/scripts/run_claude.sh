#!/usr/bin/env bash
# jstack-schedule-claude — deterministic launcher for headless Claude Code (`claude -p`).
...
LOGDIR="$HOME/.hermes/logs/jstack-schedule-claude"
...
{
  echo "=================================================================="
  echo "jstack-schedule-claude  @ $(date '+%Y-%m-%d %H:%M:%S %Z')"
  echo "  dir     : $DIR"
  echo "  model   : $MODEL"
  echo "  mode    : $MODE"
  echo "  session : $SID"
