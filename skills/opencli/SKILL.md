...
description: >-
  ...
  9222), never the Browser Bridge extension. Use when Jono says /opencli, "use opencli", "run
...
```bash
~/.claude/skills/opencli/scripts/opencli-cdp.sh <site> <command> [args] -f json
```
...
- **Routes to:** `~/builds/OpenCLI/AGENTS.md` (repo internals), the repo's `opencli-adapter-author` /
  ...
  Brave onto :9222).
- **Specialized sibling:** `linkedin-salesnav` already wraps the LinkedIn Sales Navigator
...
| `linkedin-salesnav` | The task is LinkedIn Sales Navigator reply-checking or lead flows — use the specialized sibling instead of raw OpenCLI commands. |
