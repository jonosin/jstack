...
After editing `docs/strategy/`, regenerate + lint the index:
    ...
    python3 ~/jstack/skills/init/scripts/docs_index.py index --write --repo . --satellite-kind build
    python3 ~/jstack/skills/init/scripts/docs_index.py lint --repo . --satellite-kind build

For each new durable artifact, copy `docs/_template.md` and follow its summary/frontmatter guidance.
...
| Date | Decision | Why |
|---|---|---|
| {today} | Scaffolded `{name}` as a build repo (init/new_build). | New build spun up; standard build tree + brain coupling (hub page + satellite registry). |
"""

# A starter for new canonical strategy docs.  It intentionally omits `title`:
# the filename is the index label, while `title` remains optional compatibility
# metadata accepted by docs_index.py.
DOC_TEMPLATE_TMPL = """---
...
def scaffold_repo(repo: Path, name: str, slug: str, one_liner: str,
    ...
    if do_git:
        ...
        r = run(["git", "commit", "-m",
                 f"chore: scaffold {name} build repo (init/new_build)"], cwd=repo)
        if r.returncode == 0:
...
def main() -> None:
    ...
    print()
    print("What next:")
    print("  - Copy docs/_template.md into docs/strategy/ for durable strategy/architecture rationale; follow its summary guidance.")
    print("    Regenerate docs/index.md after edits; specs/tickets stay in the configured tracker (.scratch/ for local tracker).")
    print("    The brain sees only the generated docs-index symlink.")
    ...
    print("        --skill prototype --skill to-tickets --skill code-review --skill triage \\")
