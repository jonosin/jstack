...
class BrainLinkTests(unittest.TestCase):
    ...
    def test_explicit_slug_overrides_human_readable_repo_name_everywhere(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("index --write --repo . --slug fopus-studyworld --satellite-kind build", output)
    ...
    def test_strategy_index_uses_filename_and_title_is_optional(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo, _brain, _link = self.make_fixture(Path(tmp))
            docs = repo / "docs"
            strategy = docs / "strategy" / "2026-07-25-routing-contract.md"
            strategy.parent.mkdir(parents=True)
            strategy.write_text(
                '---\nsummary: "Route durable work through one deterministic registry."\n'
                "status: accepted\n---\n\n# A human heading that is not the link label\n"
            )
            ...
            self.assertEqual(linted.returncode, 0, linted.stdout + linted.stderr)
            output = (docs / "index.md").read_text()
            self.assertIn("index --write --repo . --slug example-venture", output)
            self.assertIn(
                "[2026-07-25-routing-contract.md](strategy/2026-07-25-routing-contract.md)",
                output,
            )
            self.assertNotIn("[A human heading that is not the link label]", output)

    def test_canonical_strategy_summary_is_one_sentence_and_at_most_160_characters(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo, _brain, _link = self.make_fixture(Path(tmp))
            docs = repo / "docs"
            strategy = docs / "strategy" / "2026-07-25-invalid-summary.md"
            strategy.parent.mkdir(parents=True)
            strategy.write_text(
                '---\nsummary: "This sentence is valid. This second sentence is not."\n'
                "status: draft\n---\n\n# Invalid summary\n"
            )
            self.assertEqual(
                self.run_cli("index", "--write", "--repo", str(repo)).returncode, 0
            )
            linted = self.run_cli("lint", "--repo", str(repo))
            self.assertNotEqual(linted.returncode, 0)
            self.assertIn("must be exactly one sentence", linted.stdout)
            ...
            linted = self.run_cli("lint", "--repo", str(repo))
            self.assertNotEqual(linted.returncode, 0)
            self.assertIn("160 characters or fewer", linted.stdout)
    ...
    def test_legacy_summary_contract_warns_without_blocking(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo, _brain, _link = self.make_fixture(Path(tmp))
            docs = repo / "docs"
            strategy = docs / "strategy" / "2026-07-25-valid-summary.md"
            strategy.parent.mkdir(parents=True)
            strategy.write_text(
                '---\nsummary: "A valid strategy summary."\nstatus: accepted\n---\n\n# Current\n'
            )
            legacy = docs / "build" / "old-summary.md"
            legacy.parent.mkdir(parents=True)
            legacy.write_text(
                '---\nsummary: "This preserved record has two sentences. It remains historical."\n'
                "status: archived\n---\n\n# Old\n"
            )
            self.assertEqual(
                self.run_cli("index", "--write", "--repo", str(repo)).returncode, 0
            ...
            linted = self.run_cli("lint", "--repo", str(repo))
            self.assertEqual(linted.returncode, 0, linted.stdout + linted.stderr)
            self.assertIn("WARN build/old-summary.md: legacy summary", linted.stdout)
    ...
    def test_new_venture_scaffold_wires_the_brain_visible_index(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((docs / "_template.md").exists())
            index_output = (docs / "index.md").read_text()
            self.assertIn("root `AGENTS.md`", index_output)
            self.assertIn("Summary guidance: read `docs/_template.md`", index_output)
            template = (docs / "_template.md").read_text()
            self.assertIn("factual, specific cue", template)
            self.assertIn("Good:", template)
            self.assertIn("Bad:", template)
            self.assertFalse((docs / "build").exists())
            self.assertFalse((docs / "scratch").exists())
            self.assertFalse((docs / "superpowers").exists())
            self.assertFalse((docs / "AGENTS.md").exists())
            self.assertFalse((docs / "CLAUDE.md").exists())
            root_router = (root / "ventures" / "example-venture" / "AGENTS.md").read_text()
            self.assertNotIn("/savetobrain", root_router)
            self.assertNotIn("/brainwork", root_router)
            self.assertNotIn(".agents/skills", root_router)
            self.assertIn("**Read flow:**", root_router)
            self.assertIn("work-grade", root_router)
            self.assertIn("docs/_template.md", root_router)
            self.assertIn("docs_index.py", root_router)
            self.assertNotIn("24 words", root_router)
            self.assertTrue((root / "ventures" / "example-venture" / "clients" / ".gitkeep").exists())
            self.assertEqual(list((root / "ventures" / "example-venture" / "clients").iterdir()),
                             [root / "ventures" / "example-venture" / "clients" / ".gitkeep"])
    ...
    def test_new_build_scaffold_wires_docs_layout_and_build_index_symlink(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((docs / "_template.md").is_file())
            template = (docs / "_template.md").read_text()
            self.assertIn("factual, specific cue", template)
            self.assertIn("Good:", template)
            self.assertIn("Bad:", template)
            ...
            index_output = (docs / "index.md").read_text()
            self.assertIn("Summary guidance: read `docs/_template.md`", index_output)
            root_router = (repo / "AGENTS.md").read_text()
            self.assertIn("docs/_template.md", root_router)
            self.assertIn("docs_index.py", root_router)
