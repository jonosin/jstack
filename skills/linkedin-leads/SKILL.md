...
Use `linkedin-send` after a final `prospects.csv` exists and needs validation/sending.
...
7. Hand the final outreach CSV to `linkedin-send`.
...
| Next | When |
|------|------|
| `linkedin-send` | A final `prospects.csv` exists and needs validation/sending. |
| `linkedin` | Unsure whether leads should come from here vs Sales Navigator. |
