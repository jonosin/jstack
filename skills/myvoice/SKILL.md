...
metrics, opener shapes, cadence) lives in `gtmarketing` → `references/cold-outreach.md`, not here.
...
| Next | When |
|------|------|
| `/linkedin-send` | The voiced draft is a LinkedIn InMail/DM ready to actually send. |
| `/gtmarketing` | The draft needs cold-outbound STRATEGY first (references/cold-outreach.md) or sits inside a broader GTM motion. |
| `/savetobrain` | A voice correction/decision emerged that should persist beyond the canon files. |
