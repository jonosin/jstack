...
# imgen — generic Gemini image generation wrapper
...
image models. This is NOT for image analysis/understanding (use `vision`
...
- Analyze/describe images (use `vision` / `gv` for that)
- Handle image-to-image transformation where the image IS the input (use `vision`)
...
| Next | When |
|---|---|
| `/vidgen` | Animate a generated still into a video clip (single-image i2v) — the universal video-gen router. |
