# Packaging jstack skills — authoring convention

## Naming convention

- Use a short kebab-case name (`savetobrain`, `vision`, `handoff`). The plugin namespace is the
  suite identity: `jstack-skills:<skill>`.
- Keep venture-specific skills in their venture repository. Never use a colon in a skill directory
  name; the harness uses it for a plugin namespace.

## Repo-local family (default for a new venture or project family)

A new venture skill family belongs to its **target repo only** unless the user explicitly asks for
global promotion. It is not a new member of the global jstack suite.

1. Read the target repo's `AGENTS.md` and use its local-skill convention. If it has none, create the
   family under `<target-repo>/.agents/skills/`: a router and leaf skills as sibling directories.
2. Keep every family directory and helper inside the target repo. Add the family's location to that
   repo's agent instructions only when its local convention requires registration.
3. Do **not** run `new-jstack-skill.sh`: it creates a global-suite skill and harness symlinks. Do not
   add a repo-local family to `~/jstack/skills`, the jstack `.plugin`, or any global path under
   `~/.agents/skills`, `~/.claude/skills`, `~/.codex/skills`, or `~/.hermes/skills`.
4. Validate the new family with the target repo's checks. Use this suite's frontmatter and
   `## Next skills` conventions where compatible, but do not make global availability a prerequisite.
5. Promote only on an explicit user request. At promotion time, review whether the family is truly
   venture-agnostic; then move/adapt it into the global suite and use `new-jstack-skill.sh` to create
   the deliberate harness links.

This boundary prevents one venture's operating procedure from silently appearing in every project or
global skill picker.

All jstack skills are **canonical in the jstack repo** (`skills/<name>/`, wherever
you cloned jstack) and symlinked into every harness. **Every link points directly
at the repo dir — no transitive hop through `.agents`:** `~/.agents/skills/<name>`
(the discovery hub), `~/.claude/skills/<name>`, and `~/.codex/skills/<name>` each
resolve straight to `<clone>/skills/<name>`. Hermes entries live inside the package
dir `~/.hermes/skills/suite/<name>`, also pointing straight at the repo. There is
no second forked copy — edit the file in the repo and every harness sees it through
its own direct symlink.

Discord `/skill` discovery does not depend on this layout: it is handled by adding
`~/jstack/skills` to Hermes `external_dirs` (see the Discord section below), which
scans the canonical repo directly.

When asked to create a new jstack skill, do not hand-create directories or symlinks. Run:

```bash
<your-clone>/skills/suite/scripts/new-jstack-skill.sh <skill-name> --desc "one-line description"
```

It creates the canonical dir + a starter `SKILL.md` in the repo's `skills/`, then
symlinks it (direct to the repo) into `.agents`, `.claude`, `.codex`, and the
`.hermes/skills/suite/` package — skipping any harness not installed. The script
resolves the repo from its own location, so it works wherever you cloned jstack. It
is idempotent: re-running it on an existing skill repairs or retargets symlinks
without touching the canonical `SKILL.md`.
After running it, edit the canonical `SKILL.md` in the repo to fill in the body.

## Frontmatter rules (Cowork `.plugin` validator)

The family is packaged as a `.plugin` for Claude Cowork, and its validator rejects the
**entire bundle** if any single skill breaks a rule. Every skill's frontmatter must satisfy:

- **`name` has no reserved word** (`claude`, `anthropic`). Refer to Claude Code as `cc` —
  e.g. use `schedule-cc` (the suffix that would spell out the model name is banned).
- **`name` is kebab-case, ≤64 chars, and equals the skill's directory name.**
- **`description` is ≤1024 characters.**
- **`description` has no angle brackets.** `<slug>` parses as an XML tag and fails — write
  placeholders as `[slug]`.

## Build the `.plugin` (deterministic)

The Cowork bundle is regenerated by one script — never hand-zip it:

```bash
bash skills/suite/scripts/build-plugin.sh --bump patch
```

What it does, in order, with no per-run judgment:

1. Reads the manifest at `<repo>/.claude-plugin/plugin.json` (the single source of truth for plugin
   `name` + `version`). `--bump patch|minor|major` or `--version X.Y.Z` writes the new version back to
   the manifest so a Cowork reinstall registers as an update; omit them to build the current version.
2. **Auto-discovers** every `skills/<name>/` containing a `SKILL.md`. New and updated skills are
   picked up automatically — there is no skill list to maintain. Directories without a `SKILL.md`
   (e.g. an absorbed husk like `coldmsg`) are skipped and reported.
3. Stages a copy and **scrubs non-shippable junk**: `.git`, `.venv`, `node_modules`, `__pycache__`,
   `*.pyc`, `*.orig`, `*.bak`, `.DS_Store`, and dangling symlinks.
4. **Validates every skill's frontmatter** against the Cowork `.plugin` rules above (name==dir,
   kebab-case, ≤64, no reserved word, description present + ≤1024 + no angle brackets). Any violation
   fails the build — the whole bundle is rejected, matching the real validator's all-or-nothing behavior.
5. Zips with stable, sorted file ordering to `dist/<plugin-name>.plugin` (override with `--out`).

Use `--print-only` to discover + validate without writing (a fast pre-flight). In a Cowork session,
pass `--out` at the outputs directory and then present the resulting file.

## Next skills table (required on every skill)

Every jstack skill ENDS with a `## Next skills` table — the recommended skill(s) to invoke
after this one, so an agent (or Jono) always knows the next hop in a workflow. Two columns:
the skill (an invocable skill or, for craft, `video-prod-skills:…`) and **when** to go
there. List only real next hops, not a catalog. A standalone skill with no natural successor
still includes the table — name companion/related skills or state "standalone — no required
next step." The scaffolder seeds an empty version; fill it when you write the body.

> **Video generation is always routed through `/vidgen`** — any skill whose workflow
> reaches "generate a video / clip" points its Next-skills row at `/vidgen` (the
> universal router that enforces ask-duration+resolution, show-the-exact-prompt-and-craft-skill,
> and no dup-frame), never directly at a backend.

## Before pushing

The repo is shareable. Before any `git push`, run `tools/secrets-gate.sh` from the
repo root — it scans for personal identity, machine paths, GCP project ids, and key
patterns, and exits non-zero on a hit. Keep personal values in `~/.jstack/config.env`
(gitignored), never in a committed skill. Skills read config at runtime via
**env var → `~/.jstack/config.env` → default**.

Run `scripts/skill-lint.sh <skill-dir>` on any skill you changed — it enforces frontmatter, trigger-style description, the Next-skills table, no absolute paths, and the SKILL.md size budget.

## Discord `/skill` autocomplete and symlinks

Hermes' Discord gateway resolves skill symlinks to their real paths with
`Path.resolve()` and drops any skill whose resolved path falls outside a configured
scan root. Since jstack skills are canonical in `~/jstack/skills/` and only symlinked
into harness directories, the resolved path lands outside both `~/.hermes/skills/` and
`~/.agents/skills/`, causing the skill to be silently absent from Discord's `/skill`
picker.

The fix is to add `~/jstack/skills` as an external skills directory in Hermes config:

```yaml
skills:
  external_dirs:
    - ~/.agents/skills
    - ~/jstack/skills
```

After updating config, restart the gateway or run `/restart` on Discord. Without this,
`hermes skills list` and `skill_view` will see the skills but `/skill` on Discord won't.

## Adapting external skills

When adapting a skill from an external source (gstack, skills.sh, another agent's repo), do not copy it verbatim. External skills are often deeply coupled to their host infrastructure. The gstack `skillify` skill, for example, is 1,200 lines where 60% is gstack-specific preamble, onboarding state machines, telemetry, the `$B` browser harness, `browse-client.ts` SDK, and atomic write helpers. None of that ports.

The right approach: read the full source, extract the core workflow (steps that describe the actual transformation, not the host plumbing), map every host-specific tool reference to its Hermes equivalent, and drop anything that assumes a different agent runtime. Scaffold with `new-jstack-skill.sh` and write the SKILL.md body from the extracted workflow, not from the source file.
