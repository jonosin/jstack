...
| Marketing video production (route actual generation via `/jstack-vidgen`) | `video` |
...
- `jstack-coldmsg` is retired (2026-07-06); its doctrine lives in `references/cold-outreach.md`
  here. If old notes/handoffs invoke `/jstack-coldmsg`, Read that reference instead.

## Next skills

| Next | When |
|------|------|
| `/jstack-myvoice` | Any outbound copy is drafted and needs the user's voice pass (cold copy is drafted in-skill via `references/cold-outreach.md`). |
...
| `/jstack-savetobrain` | A durable GTM/marketing decision came out and should persist to the brain. |
| `/jstack-vidgen` | A phase/playbook reaches "generate a video/clip". |
