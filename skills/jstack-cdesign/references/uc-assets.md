...
to Jono's image pipeline. Claude Code can — via `/jstack-imgen` (Gemini image gen) for images, and the
StayFrame / `/jstack-vidgen` pipeline for video. This use case generates assets locally and feeds them
...
- **Claude Code generates** the assets: `/jstack-imgen` for images/sprites/textures (supports reference
  images + chroma-key post-processing), `/jstack-vidgen` for any video/clip. These are the things CD
  cannot make.
...
2. **Generate locally.** Run `/jstack-imgen` (or `/jstack-vidgen` for clips). Save into the package dir as
