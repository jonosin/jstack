---
name: jstack-savetobrain-from-cc
description: Extract a Claude Code session transcript from the second-brain Claude project as a raw drop, ready for /jstack-brainwork to ingest. Reuses the jstack transcript extractor but writes to raw/personal/drops/ (or raw/personal/scratch/ with --staging). Use when the user says /jstack-savetobrain-from-cc, save this Claude session to the brain, capture the current Claude session, or wants a Claude Code transcript preserved as second-brain raw material.
user_invocable: true
---

# jstack-savetobrain-from-cc
...
python3 ~/.claude/skills/jstack-savetobrain-from-cc/scripts/extract_jstack_transcript.py <session-id>
```
...
| `/jstack-cc-find` | If you still need to locate the session ID to extract. |
